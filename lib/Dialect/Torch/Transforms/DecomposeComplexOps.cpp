...
namespace {
// Decompose `aten.index_add` op into `aten.scatter_add`
...
public:
  ...
  LogicalResult matchAndRewrite(AtenIndexAddOp op,
    ...
    auto resType = cast<BaseTensorType>(op.getType());
    auto srcType = cast<BaseTensorType>(src.getType());
    ...
    rewriter.replaceOpWithNewOp<Torch::AtenScatterAddOp>(
        op, resType, input, op.getDim(), index, newSrc);
    return success();
  }
};
} // namespace
