...
#define GEN_PASS_DEF_RAISEIFOPSHAPESDTYPES
#include "torch-mlir/Dialect/Torch/Transforms/Passes.h.inc"

namespace {
class RaiseIfOpShapesDtypes : public OpRewritePattern<PrimIfOp> {
    ...
    for (auto result : llvm::enumerate(ifOp.getResults())) {
      ...
      auto newResult =
          copyTensorToType(rewriter, ifOp.getLoc(),
                           cast<BaseTensorType>(oldType), result.value());
...
class RaiseIfOpShapesDtypesPass
    : public impl::RaiseIfOpShapesDtypesBase<RaiseIfOpShapesDtypesPass> {
  void runOnOperation() override {
    MLIRContext *context = &getContext();
    auto func = getOperation();

    RewritePatternSet patterns(context);
    patterns.insert<RaiseIfOpShapesDtypes>(context);
